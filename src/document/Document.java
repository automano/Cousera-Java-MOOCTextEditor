...
public abstract class Document {
     ...
     * 	1. Add 1 every time the letter 'y' makes the sound of a vowel (A, E, I, O, U).
     *  
     *  2. Subtract 1 for each silent vowel (like the silent 'e' at the end of a word).
     *  
     *  3. Subtract 1 for each diphthong or triphthong in the word.
     *     		Diphthong: when 2 vowels make only 1 sound (au, oy, oo)
     *    		Triphthong: when 3 vowels make only 1 sound (iou)
     *  4. Does the word end with "le" or "les?" Add 1 only if the letter before the "le" is a consonant.
     *  
	...
	public double getFleschScore()
	{
		...
		double wordsPerSentences = (float) getNumWords() / (float) getNumSentences();
		double syllablesPerWords = (float) getNumSyllables() / (float) getNumWords();
		
		fleschScore = 206.835 - (1.015 * wordsPerSentences) - (84.6 * syllablesPerWords);
		return fleschScore;
	}
